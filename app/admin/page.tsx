...
type Claim = {
  ...
  blogUrl: string;
  status: string;
  revivalPostUrl: string | null;
  revivalPostTitle: string | null;
  claimVerified: boolean;
  claimant: { handle: string; displayName: string; email: string | null };
};
...
export default function AdminPage() {
  ...
  async function verify(bloggerId: string, verified: boolean) {
    setBusyId(bloggerId);
    setError(null);
    const res = await verifyClaim(bloggerId, verified);
    setBusyId(null);
    if (res?.error) setError(res.error);
    else
      setClaims((cs) =>
  ...
  return (
    <main className="min-h-screen bg-neutral-50 px-6 py-10 text-neutral-900">
      <div className="mx-auto max-w-2xl">
        ...
        {isAdmin && <h2 className="mt-8 text-lg font-bold">Nominations</h2>}

        {isAdmin && noms !== null && noms.length === 0 && (
          <p className="mt-3 text-sm text-neutral-500">No pending nominations. 🎉</p>
        )}
        ...
        {isAdmin && <h2 className="mt-10 text-lg font-bold">Profile claims</h2>}

        {isAdmin && claims !== null && claims.length === 0 && (
          <p className="mt-3 text-sm text-neutral-500">No profile claims yet.</p>
        )}
            ...
            {claims.map((c) => (
              ...
              >
                <div className="flex flex-wrap items-baseline gap-x-2">
                  ...
                  {c.blogName && c.blogName !== c.name && (
                    <span className="text-sm text-neutral-500">· {c.blogName}</span>
                  )}
                  <span
                  ...
                  >
                    {c.claimVerified ? "verified" : "pending"}
                  </span>
                </div>
                <p className="mt-1 text-sm text-neutral-700">
                  claimed by {c.claimant.displayName} (@{c.claimant.handle})
                ...
                <p className="mt-1 text-sm">
                  <a
                    ...
                    className="text-blue-700 underline"
                  >
                    {c.blogUrl.replace(/^https?:\/\/(www\.)?/, "")}
                  </a>
                  {c.revivalPostUrl ? (
                    <>
                      {" · "}
                      <a
                        href={c.revivalPostUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-blue-700 underline"
                      >
                        revival post{c.revivalPostTitle ? `: ${c.revivalPostTitle}` : ""}
                      </a>
                    </>
                  ) : (
                    <span className="text-neutral-400"> · no revival post linked yet</span>
                  )}
                </p>
                <div className="mt-3 flex gap-2">
                  {c.claimVerified ? (
