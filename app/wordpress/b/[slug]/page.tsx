  ...
  if (!blogger) {
    return (
      <div>
        ...
        <p className="mt-2">
          No bounty here. <Link href="/wordpress">← Back to the front page</Link>
        </p>
      </div>
    );
  }
    ...
    <article>
      ...
      <section className="mt-6">
        ...
        <p className="wp-meta mt-2">
          ...
          <Link href="/wordpress/claim">Claim this profile</Link> to direct the
          bounty.
        </p>
      </section>
            ...
            {pledges.slice(0, 10).map((p: any) => (
              <tr key={p.id} className="border-b border-dotted border-wpborder">
                <td className="py-1.5 pr-2">
                  {p.patron ? (
                    <Link href={`/wordpress/p/${p.patron.handle}`}>
          ...
          {comments.map((c: any, i: number) => (
            ...
            >
              <p className="text-[12px] font-bold">
                {c.author ? (
                  <Link href={`/wordpress/p/${c.author.handle}`}>
      ...
      <p className="mt-8">
        <Link href="/wordpress">« Back to all bounties</Link>
      </p>
    </article>
  );
}
