...
function SignInInner() {
  ...
  async function sendCode() {
    ...
    try {
      ...
      posthog.capture("magic_code_requested", { method: "email", skin: "graveyard" });
      setStage("code");
    } catch (e: any) {
  ...
  async function verify() {
    ...
    try {
      ...
      posthog.capture("signed_in", { method: "magic_code", skin: "graveyard" });
      router.push(next);
    } catch (e: any) {
    ...
    <main className="mx-auto max-w-md px-6 py-20 text-center">
      ...
      {stage === "email" && (
        <>
          <a
            ...
            className="gy-caps mt-8 block w-full rounded-sm border border-candle/60 px-6 py-3 text-[19px] text-candle hover:bg-candle/10 aria-disabled:pointer-events-none aria-disabled:opacity-50"
          >
      ...
      >
        ...
        <button
          type="submit"
          disabled={busy}
          className="gy-caps w-full rounded-sm bg-candle px-6 py-3 text-[19px] text-night hover:bg-candle/90 disabled:opacity-50"
        >
